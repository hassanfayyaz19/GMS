...
<!--.row-->
<form action="conf_joborder_stock_issue.php?chkp=<?php echo $_GET['chkp'];?>&m=<?php echo $_GET['m'];?>&mid=<?php echo $_GET['mid'];?>" method="post" enctype="multipart/form-data">
                        ...
                        <div id="Mainsizer">
							...
							<div class="col-md-12">
                                ...
                                <select relqty="<?php echo $newQty;?>" reluom="<?php echo $sqlItemD[0]['uom_id'];?>" relasid="<?php echo $sqlsizeD['assignment_size_id'];?>" reli="<?php echo $i;?>" class="select2 m-b-10 select2-multiple selectedstock" name="stocks<?php echo $k;?>[]" multiple="multiple" data-placeholder="Choose">
									...
									{
										$sqlstockItems="SELECT * FROM tbl_stock_items,tbl_stock_item_rolls WHERE tbl_stock_items.stock_item_id=tbl_stock_item_rolls.stock_item_id AND tbl_stock_items.stock_item_id=".$sqlMaterialD['stock_item_id']."";
									...
									{
										$sqlstockItems="SELECT * FROM tbl_stock_items WHERE tbl_stock_items.stock_item_id=".$sqlMaterialD['stock_item_id']." AND stock_quantity<>0";
										$sqlstockItemsQ=$db->record_select($sqlstockItems);
										foreach($sqlstockItemsQ as $sqlstockItemsD)
										{
										?>
									...
									?>
                                                                        ...
                                                                        <td>
                                                                        <input type="hidden" name="h_total[]" id="h_total_<?php echo $sqlsizeD['assignment_size_id'];?>_<?php echo $i;?>" />
                                                                        <input class="form-control" id="total_<?php echo $sqlsizeD['assignment_size_id'];?>_<?php echo $i;?><?php //echo $sqlsizeD['assignment_size_id'];?>" name="total[]" readonly /></td>
                                                                        <td>
                                                                        <input type="hidden" name="h_remaining[]" id="h_remaining_<?php echo $sqlsizeD['assignment_size_id'];?>_<?php echo $i;?>" />
                                                                        <input class="form-control" id="remaining_<?php echo $sqlsizeD['assignment_size_id'];?>_<?php echo $i;?><?php //echo $sqlsizeD['assignment_size_id'];?>" name="remaining[]" readonly /></td>
																	</tr>
																	<?php $i++; $k++;}?>
																</tbody>
															</table>
...
$(document).ready(function() {
		...
		$(document).on("change",".selectedstock",function(){
			...
			//alert(thisval);
			$.ajax({
			url: 'get_total_selected_stock.php',
			type: 'POST',
