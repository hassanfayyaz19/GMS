<div class="row bg-title">
    <div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
        <h4 class="page-title">List of Size</h4> </div>
...
$(document).ready(function() {
	...
	$('#demo-foo-pagination').DataTable({
		...
		'serverMethod': 'post',
		'ajax': {
			'url':'sizeajax.php?chkp=<?php echo $_GET['chkp'];?>'
		},
		'columns': [
			{ data: 'size' },
			{ data: 'size_short' },
	...
	$(document).on("click",".delete",function(){
		...
		swal({   
			...
			closeOnConfirm: false 
				...
				url: 'conf_delete.php',
				type: 'POST',
				data: { size_id:mid, cmdType:"delsizeSingle"}, // it will serialize the form data
					dataType: 'html'
				})
				.done(function(data){
					 var resultArray = jQuery.parseJSON(data);
					 var msg=resultArray['msg'];
					 var size=resultArray['size'];
					 //alert(data);
					 if(msg==1)
					 {
					 	swal("congratulations!", "'"+size+"' Removed Successfully.", "success"); 
						var oTable = $('#demo-foo-pagination').DataTable( );
						oTable.ajax.reload();// to reload
					 }
					 else
					 {
					 	swal("Oops!", "Error While removing '"+size+"', Please try later.", "error"); 
					 }
				})
