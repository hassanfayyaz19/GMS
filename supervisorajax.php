...
$sel = "select * from tbllogin, tbl_users WHERE tbllogin.log_id=tbl_users.log_id AND tbllogin.typ_id=4";
$totalRecords=$db->record_total($sel);

## Total number of records with filtering
$sel = "select * from tbllogin, tbl_users WHERE tbllogin.log_id=tbl_users.log_id AND tbllogin.typ_id=4 ".$searchQuery;
$totalRecordwithFilter=$db->record_total($sel);

## Fetch records
$empQuery = "select * from tbllogin, tbl_users WHERE tbllogin.log_id=tbl_users.log_id AND tbllogin.typ_id=4 ".$searchQuery." order by ".$columnName." ".$columnSortOrder." limit ".$row.",".$rowperpage;
...
foreach ($empRecords as $row) {
	...
	if($RoleArray['Edit']==1)
	{
		$actionhtmledit='<a type="button" class="btn btn-info btn-circle tooltip-info" data-toggle="tooltip" data-placement="top" title="" data-original-title="Edit Royalty" href="index_admin.php?chkp=392&amp;cmdType=edit&mid='.$row['log_id'].'&m=129"><i class="fa fa-pencil"></i> </a> &nbsp;';
