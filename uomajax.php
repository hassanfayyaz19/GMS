...
$searchQuery = " ";
if($searchValue != ''){
	$searchQuery = " and ((uom like '%".$searchValue."%') OR (uom_status like '%".$searchValue."%'))";
}
...
## Fetch records
$empQuery = "select * from tbl_uom WHERE 1 ".$searchQuery." order by `uom_id` DESC,".$columnName." ".$columnSortOrder." limit ".$row.",".$rowperpage;
...
foreach ($empRecords as $row) {
	...
	if($RoleArray['Edit']==1)
	{
		$actionhtmledit='<a type="button" class="btn btn-info btn-circle tooltip-info" data-toggle="tooltip" data-placement="top" title="" data-original-title="Edit Royalty" href="index_admin.php?chkp=376&amp;cmdType=edit&mid='.$row['uom_id'].'&m=123"><i class="fa fa-pencil"></i> </a> &nbsp;';
