...
else
{
	...
	foreach($stock_item_roll_id_all as $stock_item_roll_id)
	{
		$ttlid=explode("-",$stock_item_roll_id);
		if(count($ttlid)>1)
		{
			$sqlstockItems="SELECT * FROM tbl_joborder_stock WHERE job_order_stock_id=".$ttlid[0]."";
			$sqlstockItemsQ=$db->record_select($sqlstockItems);
			$totalAmount=$totalAmount+$sqlstockItemsQ[0]['assigned_qty'];
		}
		else
		{
