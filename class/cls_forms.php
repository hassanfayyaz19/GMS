...
$id=$selPre;
	...
	$this->sList="<select name=\"".$selName."\" id=\"".$selName."\" class=\"$selClass\" onChange=\"".$selFun."\" size=\"$selSize\" multiple >";
		...
		if($selWv==''){
				$sql ="SELECT * FROM $selTable order by ".$selPre." ".$AscDesc;
				$result=$this->cdb->record_select($sql);

			}else{
				$sql ="SELECT * FROM $selTable where $selWv order by ".$selcap." ".$AscDesc;
				$result=$this->cdb->record_select($sql);
		}
	if($int_cap<>""){
...
$id=$selPre;
	...
	$this->sList="<select name=\"".$selName."\" id=\"".$selName."\" class=\"$selClass\" onChange=\"".$selFun."\" size=\"$selSize\" multiple $d >";
		...
		if($selWv==''){
				$result = mysql_query("SELECT * FROM $selTable order by ".$selPre) or die(mysql_error());

			}else{

				$result = mysql_query("SELECT * FROM $selTable where $selWv order by ".$selcap) or die(mysql_error());
		}
	if($int_cap<>""){
	$this->sList .= "<option value=\"0\" selected>".$int_cap."</option>";
	}
	 while($code = mysql_fetch_object($result)) {
...
$id=$selPre;
	...
	$this->sList="<select name=\"".$selName."\" class=\"textfield1\" onChange=\"".$selFun."\" size=\"$selSize\" multiple >";
		...
		if($selWv==''){
				$result = mysql_query("SELECT * FROM $selTable order by link_rank ") or die(mysql_error());

			}else{
				$result = mysql_query("SELECT * FROM $selTable where $selWf='$selWv' order by link_rank ") or die(mysql_error());
		}
...
$id=$selPre;
	...
	$this->sList="<select name=\"".$selName."\" class=\"textfield1\" onChange=\"".$selFun."\" size=\"$selSize\" multiple >";
		...
		if($selW==''){
				...
				$result = mysql_query("SELECT * FROM $selTable where $selW order by ".$selPre) or die(mysql_error());
		}
...
function selectgraphics($selName, $selFun, $selSize, $selClass, $selSel, $qry){
	...
	$this->sList_gra .= "<option value=\"0^0\" selected>No Graphics</option>";
	 while($code = mysql_fetch_array($qry)) {
	 $w=$code[1];
	 $h=$code[2];
	 $p=$code[3];
	 $price=($w*$h)*$p;
...
$id=$selPre;
	...
	$this->sList="<select name=\"".$selName."\" class=\"textfield1\" onChange=\"".$selFun."\" size=\"$selSize\" multiple >";
		...
		if($selWv==''){
				$result = mysql_query("SELECT ".$id.",".$name."  addr FROM $selTable where reg_sts = 'A' order by ".$selPre) or die(mysql_error());

			}else{
				$result = mysql_query("SELECT  ".$id.",".$name." addr FROM $selTable where $selWf='$selWv' and reg_sts = 'A' order by ".$selPre) or die(mysql_error());
		}
...
$this->mType ="";
$result = mysql_query("SELECT * FROM $t where $f = '".$id."'") or die(mysql_error());
...
function demail(){
$result = mysql_query("SELECT * FROM tblconfig  where id=1") or die(mysql_error());
while($rstRow = mysql_fetch_array($result))
	{
...
$qr=mysql_query("select * FROM tblmenu where men_master_id ='".$record['men_id']."' order by men_order");
		$nrs=mysql_num_rows($qr);
		if($nrs>0){
...
$ressqlMenu1=mysql_query($sqlMenu1);
$i=1;
while($record1=mysql_fetch_array($ressqlMenu1))
{
...
$qr=mysql_query($q);
		$nrs=mysql_num_rows($qr);
		while ($db_field1 = mysql_fetch_array($qr) ) {
// for same as above
...
men_master_id=0 AND men_place=1 ORDER BY men_order";
  ...
  while($row=mysql_fetch_array($ressqlmenu))
  {
     $alias=$row["men_alias"];
...
$record=mysql_fetch_array($sqlres);
$msg=$record["con_body"];
...
<br />
';
while($resrec=mysql_fetch_array($recsql))
{
