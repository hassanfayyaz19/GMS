	...
	if(isset($_REQUEST["pid"]))                    //if pid is not set mean default selection
	{
		if($_REQUEST["pid"]==$record["men_id"])    // for menu Selection 
		{
		 ...
		 else
		 {
				...
				$SqlCQ=mysqli_query($SqlC);
				while($SqlCD=mysqli_fetch_array($SqlCQ))
				{
...
{
	if($event_status=='N' || $event_status=='D' || $event_status=='0' || $event_status=='Disable')//$event_status=="" || 
		$returnHtml='<input name="'.$event_status_name.'" id="'.$event_status_name.'" type="checkbox" class="js-switch" data-color="#6164c1" />';
	else
		$returnHtml='<input name="'.$event_status_name.'" id="'.$event_status_name.'" type="checkbox" checked class="js-switch" data-color="#6164c1" />';
	return $returnHtml;
}
