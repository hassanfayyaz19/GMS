<div class="row bg-title">
    <div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
        <h4 class="page-title">List of UOM</h4> </div>
...
$(document).ready(function() {
	...
	$(document).on("click",".btnEdit", function(){
		var uom=$(this).attr('relname');
		var uom_id=$(this).attr('rel');
		$("#myModalLabel").html("Edit "+uom);
		$.ajax({
			url: 'get_edit_uom.php',
			type: 'POST',
			data: { uom_id:uom_id, cmdType:"edit"}, // it will serialize the form data
				dataType: 'html'
			})
			.done(function(data){
	...
	$('#demo-foo-pagination').DataTable({
		...
		'serverMethod': 'post',
		'ajax': {
			'url':'uomajax.php?chkp=<?php echo $_GET['chkp'];?>'
		},
		'columns': [
			{ data: 'uom' },
			{ data: 'status' },
		...
		swal({   
			...
			closeOnConfirm: false 
				...
				.done(function(data){
					 ...
					 {
                         swal("Oops!", "Error While removing, Please try later.", "error");
                     }
					 else
					 {
					 	swal("Oops!", "Error While removing, Please try later.", "error");
